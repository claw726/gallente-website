<template>
    <div :class="['p-4', themeClass]">
      ...
      <section v-for="ally in allies" :key="ally.name" :class="['mb-12', 'flex', 'flex-col', 'md:flex-row', 'items-stretch', 'p-6', 'rounded-lg', 'shadow-lg', tileClass]">
        <div :class="['md:w-1/2', 'mb-4', 'md:mb-0', 'md:pr-6', 'border-gallente-green', 'p-4', 'rounded-lg', 'flex', 'flex-col', 'justify-between', 'glow-effect']">
          <div>
            <h2 :class="['text-2xl', 'font-semibold', 'mb-2', textClass]">{{ ally.name }}</h2>
        ...
        <div class="p-4 flex md:w-1/2 justify-center items-center rounded-lg">
          <img :src="ally.image" :alt="ally.name" class="w-auto rounded-lg shadow-md m-8 border-alert-green glow-effect-green" />
        </div>
  ...
  export default {
    ...
    computed: {
      ...
      gallenteAlertColor() {
        return this.themeState.theme = '#58BF9A'; // Example color for Gallente Alert
      },
      tileClass() {
  ...
  .border-alert-green {
    border: 2px solid #58BF9A; /* Example border color for allies */
  }
